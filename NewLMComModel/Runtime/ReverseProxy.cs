...
namespace LM {
  ...
  public class RewriterModule : IHttpModule {
    void IHttpModule.Init(HttpApplication context) {
      context.BeginRequest += (sender, a) => {
        var ctx = ((HttpApplication)sender).Context; var request = ctx.Request; var response = ctx.Response;
  ...
  public static class ProxyHandler {
    ...
    /// <returns></returns>
    static WebResponse SendRequestToTarget(Uri RequestUrl, Uri ResponseUrl, HttpContext context) {
      // get the request
      ...
      if (request.ContentLength >= 0 && !knownVerb.ContentBodyNotAllowed) {
        int bufferSize = Manager.RequestSize;
        using (Stream requestStream = request.GetRequestStream(), bufferStream = new BufferedStream(context.Request.InputStream, Manager.BufferSize)) {
      ...
      try { response = request.GetResponse(); } catch (WebException exc) {
    ...
    static void SendResponseToClient(Uri RequestUrl, Uri ResponseUrl, HttpContext context, WebResponse response) {
      ...
      int bufferSize = Manager.ResponseSize;
      // push the content out to through the stream
      using (var responseStream = response.GetResponseStream())
      using (var bufferStream = new BufferedStream(responseStream, Manager.BufferSize)) {
        byte[] buffer = new byte[bufferSize];
    ...
    static public void ProcessRequest(Uri RequestUrl, Uri ResponseUrl, HttpContext context) {
      Manager.Log("**********************************************************************************");

      // send the request to the target
      Manager.Log("Request: " + RequestUrl, "Proxy");
      var response = SendRequestToTarget(RequestUrl, ResponseUrl, context);

      // send the response to the client
      Manager.Log("Response: " + ResponseUrl, "Proxy");
      SendResponseToClient(RequestUrl, ResponseUrl, context, response);
  ...
  internal class KnownHttpVerb {
    private static readonly Dictionary<string, KnownHttpVerb> NamedHeaders = new Dictionary<string, KnownHttpVerb>(StringComparer.OrdinalIgnoreCase) {
  ...
  internal static class Manager {
    ...
    public static int RequestSize = 2048;
    public static int ResponseSize = 2048;
