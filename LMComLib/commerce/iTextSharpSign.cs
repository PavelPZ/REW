...
namespace LMComLib.Commerce.PDF {
  ...
  /// </summary>
  class PDFSigner {
    private MemoryStream inputPDF;
    private MemoryStream outputPDF;
    private Cert myCert;
    private MetaData metadata;

    public PDFSigner(MemoryStream input, MemoryStream output) {
      this.inputPDF = input;
      this.outputPDF = output;
    }

    public PDFSigner(MemoryStream input, MemoryStream output, Cert cert) {
      this.inputPDF = input;
      this.outputPDF = output;
      this.myCert = cert;
    }
    public PDFSigner(MemoryStream input, MemoryStream output, MetaData md) {
      this.inputPDF = input;
      this.outputPDF = output;
      this.metadata = md;
    }
    public PDFSigner(MemoryStream input, MemoryStream output, Cert cert, MetaData md) {
      this.inputPDF = input;
      this.outputPDF = output;
      this.myCert = cert;
      this.metadata = md;
    ...
    public void Sign(string SigReason, string SigContact, string SigLocation, bool visible) {
      PdfReader reader = new PdfReader(this.inputPDF.ToArray());
      //Activate MultiSignatures
      PdfStamper st = PdfStamper.CreateSignature(reader, this.outputPDF, '\0', null, true);
  ...
  public class DocAuthor {
    ...
    public DocAuthor(string author, string title, string subject, string keywords, string creator, string producer) {
      Author = author;
      Title = title;
      Subject = subject;
      Keywords = keywords;
