module vyzva {
  ...
  export interface ICoursePretestUser extends blended.IPersistNodeUser {
  ...
  export class blendedCourseTask extends blended.task {
    ...
    moveForward(ud: IBlendedCourseUser): string {
      ...
      if (childUd.url == ud.pretest.url) {
        ...
        var entryTestUser = <blended.ITestUser>childUd; if (!entryTestUser.done) return 'tasks.course.doGoAhead: !entryTestUser.done';
