module Base32 {
  ...
  export function Encoder() {
    ...
    this.readByte = function (bt:number):number {
    ...
    this.finish = function (check:boolean):string {
  ...
  Encoder.prototype.update = function (input:number[], flush:boolean):string {
  ...
  export function Decoder() {
    ...
    // to use update().
    this.readChar = function (chr:string) {
      ...
      if (skip >= 8) {
        ...
        this.output.push(bt);
        skip -= 8
        if (skip > 0) bt = (val << (5 - skip)) & 255
        else bt = 0
      }

    }

    this.finish = function (check:boolean) {
      if (skip < 0) this.output.push(alphabet[bt >> 3]);
      //var output = this.output;
      //this.output = [];
      //return output;
    }
  ...
  Decoder.prototype.update = function (input:string, flush:boolean):number[] {
