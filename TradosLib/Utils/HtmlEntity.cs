...
namespace LMNetLib {
  ...
  public class HtmlToXmlEntity {
                ...
                if (entity.Length == 0) {
                  ...
                  if (entity[0] == '#')
                    ...
                    if (o == null) {
                      ...
                      // we found one
                      code = (int)o;
                      if (code == 34 || code == 38 || code == 60 || code == 62)
                        sb.Append("&" + entity + ";");
                      else
    ...
    public static string NormalizeEntities(string text) {
      string old = text;
      while (true) {
        string nw = normalizeEntities(old);
        if (nw == normalizeEntities(nw)) return nw;
        old = nw;
      }
      throw new Exception();
    }
    static string normalizeEntities(string text) {
      if (string.IsNullOrEmpty(text)) return text;
