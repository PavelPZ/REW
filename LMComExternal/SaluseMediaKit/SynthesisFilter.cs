...
namespace Saluse.MediaKit.Decoder
{
	...
	{
		private float[] _temporaryStorage = new float[32];
		private float[] v1;
		private float[] v2;
		private float[] actual_v; // v1 or v2
		private int _actualWritePosition; // 0-15
		...
		*/
		...
		/// <summary> Compute new values via a fast cosine transform.
		/// </summary>
		private void ComputeNewValuesOld()
		{
			// p is fully initialized from x1
			//float[] p = _p;
			// pp is fully initialized from p
			...
			dest[0 + _actualWritePosition] = x1[0];
			dest[16 + _actualWritePosition] = x1[1];
			dest[32 + _actualWritePosition] = x1[2];
			dest[48 + _actualWritePosition] = x1[3];
			dest[64 + _actualWritePosition] = x1[4];
			dest[80 + _actualWritePosition] = x1[5];
			dest[96 + _actualWritePosition] = x1[6];
			dest[112 + _actualWritePosition] = x1[7];
			dest[128 + _actualWritePosition] = x1[8];
			dest[144 + _actualWritePosition] = x1[9];
			dest[160 + _actualWritePosition] = x1[10];
			dest[176 + _actualWritePosition] = x1[11];
			dest[192 + _actualWritePosition] = x1[12];
			dest[208 + _actualWritePosition] = x1[13];
			dest[224 + _actualWritePosition] = x1[14];
			dest[240 + _actualWritePosition] = x1[15];
			
			// V[16] is always 0.0:
			dest[256 + _actualWritePosition] = 0.0f;
			
			// insert V[17-31] (== -new_v[15-1]) into actual v:
			dest[272 + _actualWritePosition] = - x1[15];
			dest[288 + _actualWritePosition] = - x1[14];
			dest[304 + _actualWritePosition] = - x1[13];
			dest[320 + _actualWritePosition] = - x1[12];
			dest[336 + _actualWritePosition] = - x1[11];
			dest[352 + _actualWritePosition] = - x1[10];
			dest[368 + _actualWritePosition] = - x1[9];
			dest[384 + _actualWritePosition] = - x1[8];
			dest[400 + _actualWritePosition] = - x1[7];
			dest[416 + _actualWritePosition] = - x1[6];
			dest[432 + _actualWritePosition] = - x1[5];
			dest[448 + _actualWritePosition] = - x1[4];
			dest[464 + _actualWritePosition] = - x1[3];
			dest[480 + _actualWritePosition] = - x1[2];
			dest[496 + _actualWritePosition] = - x1[1];
		...
		private void ComputePCMSamples0(OutputBuffer buffer)
		{
			...
			float[] tmpOut = _temporaryStorage;
			int dvp = 0;
			...
			for (int i = 0; i < 32; i++)
			{
				...
				float[] dp = d16[i];
				pcm_sample = (float) (((vp[0 + dvp] * dp[0]) + (vp[15 + dvp] * dp[1]) + (vp[14 + dvp] * dp[2]) + (vp[13 + dvp] * dp[3]) + (vp[12 + dvp] * dp[4]) + (vp[11 + dvp] * dp[5]) + (vp[10 + dvp] * dp[6]) + (vp[9 + dvp] * dp[7]) + (vp[8 + dvp] * dp[8]) + (vp[7 + dvp] * dp[9]) + (vp[6 + dvp] * dp[10]) + (vp[5 + dvp] * dp[11]) + (vp[4 + dvp] * dp[12]) + (vp[3 + dvp] * dp[13]) + (vp[2 + dvp] * dp[14]) + (vp[1 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples1(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[1 + dvp] * dp[0]) + (vp[0 + dvp] * dp[1]) + (vp[15 + dvp] * dp[2]) + (vp[14 + dvp] * dp[3]) + (vp[13 + dvp] * dp[4]) + (vp[12 + dvp] * dp[5]) + (vp[11 + dvp] * dp[6]) + (vp[10 + dvp] * dp[7]) + (vp[9 + dvp] * dp[8]) + (vp[8 + dvp] * dp[9]) + (vp[7 + dvp] * dp[10]) + (vp[6 + dvp] * dp[11]) + (vp[5 + dvp] * dp[12]) + (vp[4 + dvp] * dp[13]) + (vp[3 + dvp] * dp[14]) + (vp[2 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples2(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[2 + dvp] * dp[0]) + (vp[1 + dvp] * dp[1]) + (vp[0 + dvp] * dp[2]) + (vp[15 + dvp] * dp[3]) + (vp[14 + dvp] * dp[4]) + (vp[13 + dvp] * dp[5]) + (vp[12 + dvp] * dp[6]) + (vp[11 + dvp] * dp[7]) + (vp[10 + dvp] * dp[8]) + (vp[9 + dvp] * dp[9]) + (vp[8 + dvp] * dp[10]) + (vp[7 + dvp] * dp[11]) + (vp[6 + dvp] * dp[12]) + (vp[5 + dvp] * dp[13]) + (vp[4 + dvp] * dp[14]) + (vp[3 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples3(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[3 + dvp] * dp[0]) + (vp[2 + dvp] * dp[1]) + (vp[1 + dvp] * dp[2]) + (vp[0 + dvp] * dp[3]) + (vp[15 + dvp] * dp[4]) + (vp[14 + dvp] * dp[5]) + (vp[13 + dvp] * dp[6]) + (vp[12 + dvp] * dp[7]) + (vp[11 + dvp] * dp[8]) + (vp[10 + dvp] * dp[9]) + (vp[9 + dvp] * dp[10]) + (vp[8 + dvp] * dp[11]) + (vp[7 + dvp] * dp[12]) + (vp[6 + dvp] * dp[13]) + (vp[5 + dvp] * dp[14]) + (vp[4 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples4(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[4 + dvp] * dp[0]) + (vp[3 + dvp] * dp[1]) + (vp[2 + dvp] * dp[2]) + (vp[1 + dvp] * dp[3]) + (vp[0 + dvp] * dp[4]) + (vp[15 + dvp] * dp[5]) + (vp[14 + dvp] * dp[6]) + (vp[13 + dvp] * dp[7]) + (vp[12 + dvp] * dp[8]) + (vp[11 + dvp] * dp[9]) + (vp[10 + dvp] * dp[10]) + (vp[9 + dvp] * dp[11]) + (vp[8 + dvp] * dp[12]) + (vp[7 + dvp] * dp[13]) + (vp[6 + dvp] * dp[14]) + (vp[5 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples5(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[5 + dvp] * dp[0]) + (vp[4 + dvp] * dp[1]) + (vp[3 + dvp] * dp[2]) + (vp[2 + dvp] * dp[3]) + (vp[1 + dvp] * dp[4]) + (vp[0 + dvp] * dp[5]) + (vp[15 + dvp] * dp[6]) + (vp[14 + dvp] * dp[7]) + (vp[13 + dvp] * dp[8]) + (vp[12 + dvp] * dp[9]) + (vp[11 + dvp] * dp[10]) + (vp[10 + dvp] * dp[11]) + (vp[9 + dvp] * dp[12]) + (vp[8 + dvp] * dp[13]) + (vp[7 + dvp] * dp[14]) + (vp[6 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples6(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[6 + dvp] * dp[0]) + (vp[5 + dvp] * dp[1]) + (vp[4 + dvp] * dp[2]) + (vp[3 + dvp] * dp[3]) + (vp[2 + dvp] * dp[4]) + (vp[1 + dvp] * dp[5]) + (vp[0 + dvp] * dp[6]) + (vp[15 + dvp] * dp[7]) + (vp[14 + dvp] * dp[8]) + (vp[13 + dvp] * dp[9]) + (vp[12 + dvp] * dp[10]) + (vp[11 + dvp] * dp[11]) + (vp[10 + dvp] * dp[12]) + (vp[9 + dvp] * dp[13]) + (vp[8 + dvp] * dp[14]) + (vp[7 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples7(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[7 + dvp] * dp[0]) + (vp[6 + dvp] * dp[1]) + (vp[5 + dvp] * dp[2]) + (vp[4 + dvp] * dp[3]) + (vp[3 + dvp] * dp[4]) + (vp[2 + dvp] * dp[5]) + (vp[1 + dvp] * dp[6]) + (vp[0 + dvp] * dp[7]) + (vp[15 + dvp] * dp[8]) + (vp[14 + dvp] * dp[9]) + (vp[13 + dvp] * dp[10]) + (vp[12 + dvp] * dp[11]) + (vp[11 + dvp] * dp[12]) + (vp[10 + dvp] * dp[13]) + (vp[9 + dvp] * dp[14]) + (vp[8 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples8(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[8 + dvp] * dp[0]) + (vp[7 + dvp] * dp[1]) + (vp[6 + dvp] * dp[2]) + (vp[5 + dvp] * dp[3]) + (vp[4 + dvp] * dp[4]) + (vp[3 + dvp] * dp[5]) + (vp[2 + dvp] * dp[6]) + (vp[1 + dvp] * dp[7]) + (vp[0 + dvp] * dp[8]) + (vp[15 + dvp] * dp[9]) + (vp[14 + dvp] * dp[10]) + (vp[13 + dvp] * dp[11]) + (vp[12 + dvp] * dp[12]) + (vp[11 + dvp] * dp[13]) + (vp[10 + dvp] * dp[14]) + (vp[9 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples9(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[9 + dvp] * dp[0]) + (vp[8 + dvp] * dp[1]) + (vp[7 + dvp] * dp[2]) + (vp[6 + dvp] * dp[3]) + (vp[5 + dvp] * dp[4]) + (vp[4 + dvp] * dp[5]) + (vp[3 + dvp] * dp[6]) + (vp[2 + dvp] * dp[7]) + (vp[1 + dvp] * dp[8]) + (vp[0 + dvp] * dp[9]) + (vp[15 + dvp] * dp[10]) + (vp[14 + dvp] * dp[11]) + (vp[13 + dvp] * dp[12]) + (vp[12 + dvp] * dp[13]) + (vp[11 + dvp] * dp[14]) + (vp[10 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples10(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[10 + dvp] * dp[0]) + (vp[9 + dvp] * dp[1]) + (vp[8 + dvp] * dp[2]) + (vp[7 + dvp] * dp[3]) + (vp[6 + dvp] * dp[4]) + (vp[5 + dvp] * dp[5]) + (vp[4 + dvp] * dp[6]) + (vp[3 + dvp] * dp[7]) + (vp[2 + dvp] * dp[8]) + (vp[1 + dvp] * dp[9]) + (vp[0 + dvp] * dp[10]) + (vp[15 + dvp] * dp[11]) + (vp[14 + dvp] * dp[12]) + (vp[13 + dvp] * dp[13]) + (vp[12 + dvp] * dp[14]) + (vp[11 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples11(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[11 + dvp] * dp[0]) + (vp[10 + dvp] * dp[1]) + (vp[9 + dvp] * dp[2]) + (vp[8 + dvp] * dp[3]) + (vp[7 + dvp] * dp[4]) + (vp[6 + dvp] * dp[5]) + (vp[5 + dvp] * dp[6]) + (vp[4 + dvp] * dp[7]) + (vp[3 + dvp] * dp[8]) + (vp[2 + dvp] * dp[9]) + (vp[1 + dvp] * dp[10]) + (vp[0 + dvp] * dp[11]) + (vp[15 + dvp] * dp[12]) + (vp[14 + dvp] * dp[13]) + (vp[13 + dvp] * dp[14]) + (vp[12 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples12(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[12 + dvp] * dp[0]) + (vp[11 + dvp] * dp[1]) + (vp[10 + dvp] * dp[2]) + (vp[9 + dvp] * dp[3]) + (vp[8 + dvp] * dp[4]) + (vp[7 + dvp] * dp[5]) + (vp[6 + dvp] * dp[6]) + (vp[5 + dvp] * dp[7]) + (vp[4 + dvp] * dp[8]) + (vp[3 + dvp] * dp[9]) + (vp[2 + dvp] * dp[10]) + (vp[1 + dvp] * dp[11]) + (vp[0 + dvp] * dp[12]) + (vp[15 + dvp] * dp[13]) + (vp[14 + dvp] * dp[14]) + (vp[13 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples13(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[13 + dvp] * dp[0]) + (vp[12 + dvp] * dp[1]) + (vp[11 + dvp] * dp[2]) + (vp[10 + dvp] * dp[3]) + (vp[9 + dvp] * dp[4]) + (vp[8 + dvp] * dp[5]) + (vp[7 + dvp] * dp[6]) + (vp[6 + dvp] * dp[7]) + (vp[5 + dvp] * dp[8]) + (vp[4 + dvp] * dp[9]) + (vp[3 + dvp] * dp[10]) + (vp[2 + dvp] * dp[11]) + (vp[1 + dvp] * dp[12]) + (vp[0 + dvp] * dp[13]) + (vp[15 + dvp] * dp[14]) + (vp[14 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples14(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				pcm_sample = (float) (((vp[14 + dvp] * dp[0]) + (vp[13 + dvp] * dp[1]) + (vp[12 + dvp] * dp[2]) + (vp[11 + dvp] * dp[3]) + (vp[10 + dvp] * dp[4]) + (vp[9 + dvp] * dp[5]) + (vp[8 + dvp] * dp[6]) + (vp[7 + dvp] * dp[7]) + (vp[6 + dvp] * dp[8]) + (vp[5 + dvp] * dp[9]) + (vp[4 + dvp] * dp[10]) + (vp[3 + dvp] * dp[11]) + (vp[2 + dvp] * dp[12]) + (vp[1 + dvp] * dp[13]) + (vp[0 + dvp] * dp[14]) + (vp[15 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples15(OutputBuffer buffer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				...
				//UPGRADE_NOTE: Final was removed from the declaration of 'dp '. 'ms-help://MS.VSCC.2003/commoner/redir/redirect.htm?keyword="jlca1003"'
				float[] dp = d16[i];
				pcm_sample = (float) (((vp[15 + dvp] * dp[0]) + (vp[14 + dvp] * dp[1]) + (vp[13 + dvp] * dp[2]) + (vp[12 + dvp] * dp[3]) + (vp[11 + dvp] * dp[4]) + (vp[10 + dvp] * dp[5]) + (vp[9 + dvp] * dp[6]) + (vp[8 + dvp] * dp[7]) + (vp[7 + dvp] * dp[8]) + (vp[6 + dvp] * dp[9]) + (vp[5 + dvp] * dp[10]) + (vp[4 + dvp] * dp[11]) + (vp[3 + dvp] * dp[12]) + (vp[2 + dvp] * dp[13]) + (vp[1 + dvp] * dp[14]) + (vp[0 + dvp] * dp[15])) * _scaleFactor);
		...
		private void ComputePCMSamples(OutputBuffer buffer)
		{
			switch (_actualWritePosition)
			{
				case 0: 
			...
			if (buffer != null)
			{
				buffer.AppendSamples(_channel, _temporaryStorage);
			}
		...
		public void CalculatePCMSamples(OutputBuffer buffer)
		{
			...
			_actualWritePosition = (_actualWritePosition + 1) & 0xf;
		...
		/// 
		/// </returns>
		private static float[] SubArray(float[] array, int offset, int length)
		{
