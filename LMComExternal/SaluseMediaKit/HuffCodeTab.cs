...
*  Group'query public c source for its MPEG audio decoder. Miscellaneous
	...
	internal sealed class HuffCodeTab
	{
		...
		public static int huffman_decoder(HuffCodeTab h, int[] x, int[] y, int[] v, int[] w, BitReserve br)
		{
			...
			do 
			{
				...
				{
					/*end of tree*/
					x[0] = Utilities.URShift(h.val[point][1], 4);
				...
				*/
				...
				level = Utilities.URShift(level, 1);
			...
			*/
			/* ToXmlString sign encodings for quadruples tables. */
			...
			{
				// ToXmlString sign and escape encodings for dual tables.
				// x and y are reversed in the test bitstream.
				// Reverse x and y here to make test bitstream work.
				
				if (h.linbits != 0)
					if ((h.xlen - 1) == x[0])
						x[0] += br.GetNBits(h.linbits);
				if (x[0] != 0)
					if (br.GetNextBit() != 0)
						x[0] = - x[0];
				if (h.linbits != 0)
					if ((h.ylen - 1) == y[0])
						y[0] += br.GetNBits(h.linbits);
				if (y[0] != 0)
					if (br.GetNextBit() != 0)
						y[0] = - y[0];
			}
